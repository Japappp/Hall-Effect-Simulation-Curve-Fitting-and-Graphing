...
def format_func(value, tick_number):
    return f'{value:.12f}'
path_CS = "C:\\Users\Papa\Desktop\DATA\SYM COR.txt"

fixer_CS = 0.29094663705   # FOR scaling the coefficient, so that Rl(coef=1) = 1Ohm
fixer = fixer_CS

hall_resistance_CS = 25.92325487 #Value of the resistance of the Corbino Sector from SImulation

current_device = 0.00053

# %% Making of df_CS

df_CS = pd.DataFrame(columns=['Rl', 'Theta', 'pow_0', 'pow_load', 'avg_cur',
...
list_of_thetas_for_columns_CS = []
list_of_Rl_for_columns_CS = []
...
with open(path_CS, 'r') as file:
    ...
    while i < len(lines):
        ...
        if line.startswith('% Data (pow_0 (W) @'):
            ...
            Rl = float(cof)*fixer
            list_of_thetas_for_columns_CS.append(theta)
            list_of_Rl_for_columns_CS.append(Rl)
        i = i+1


list_of_thetas_for_columns_CS_unique = list(OrderedSet(
    list_of_thetas_for_columns_CS))  
list_of_Rl_for_columns_CS_unique = list(OrderedSet(
    list_of_Rl_for_columns_CS))  

all_combinations_CS = list(itertools.product(
    list_of_Rl_for_columns_CS_unique, list_of_thetas_for_columns_CS_unique))

df_temp_CS = pd.DataFrame(all_combinations_CS, columns=['Rl', 'Theta'])
df_CS[['Rl', 'Theta']] = df_temp_CS
...
with open(path_CS, 'r') as file:
    ...
    while i < len(lines):
        ...
        if line.startswith('% Data (pow_0 (W) @'):
            ...
            df_CS.loc[(df_CS['Theta'] == theta) & (
                df_CS['Rl'] == Rl), 'pow_0'] = pow_0_value
            ...
            df_CS.loc[(df_CS['Theta'] == theta) & (
                df_CS['Rl'] == Rl), 'avg_cur'] = avg_cur_value
            ...
            df_CS.loc[(df_CS['Theta'] == theta) & (
                df_CS['Rl'] == Rl), 'volt_dif'] = volt_dif_value
            ...
            df_CS.loc[(df_CS['Theta'] == theta) & (
                df_CS['Rl'] == Rl), 'charge_acc'] = charge_acc_value
            ...
            df_CS.loc[(df_CS['Theta'] == theta) & (
                df_CS['Rl'] == Rl), 'pow_load'] = power_load_value

        i = i+1

df_CS['pow_load'] = df_CS['pow_load'].astype(float)

pow_0_CS_for_ratio = df_CS[df_CS['Theta'] ==
                           0].loc[df_CS[df_CS['Theta'] == 0]['pow_0'].idxmax(), 'pow_0']

df_CS['pow_ratio'] = df_CS['pow_load'] / float(pow_0_CS_for_ratio)

df_CS['alpha'] = (hall_resistance_CS / df_CS['Rl'])


scaling_factor_CS = df_CS[df_CS['Theta'] ==
                          0.08].loc[df_CS[df_CS['Theta'] == 0.08]['pow_load'].idxmax(), 'alpha']

df_CS['alpha_scaled'] = (df_CS['alpha'] / scaling_factor_CS)

df_CS['Rl_scaled'] = (df_CS['Rl'] / scaling_factor_CS)

df_CS = df_CS[df_CS['Theta'] > -0.01]
...
Alphas_for_few_fits = [ df_CS['alpha'][897], df_CS['alpha'][944], df_CS['alpha'][997], df_CS['alpha'][1111], df_CS['alpha'][1359] ]

Alphas_scaled_for_few_fits = [ df_CS['alpha_scaled'][372], df_CS['alpha_scaled'][440], df_CS['alpha_scaled'][750], df_CS['alpha_scaled'][997], df_CS['alpha_scaled'][1359] ]
...
plt.ylabel('Plat/P0', fontsize=25)
srednie_r2_alpha_fit = Average(r2_list_alpha_fit)
plt.title(f'Hall Bar Simulation Results for Planar Hall eEffect \n Plat/P0 vs \u03B1 at given \u03F4, average r2 = {avg_r2} \n No Offset \n FIT= a*x**2/(1+x**2) ', fontsize=30)
plt.legend(loc='lower center', bbox_to_anchor=(0.5, -0.15), ncol=4)  # Moving legend below the plot
plt.show()
plt.clf()
...
for alpha, color in zip(Alphas_for_few_fits, colors_theta_fit):
    subset= df_CS[df_CS['alpha'] == alpha]
